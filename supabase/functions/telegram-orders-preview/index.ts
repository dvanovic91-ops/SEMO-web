...
async function timingSafeEqual(a: string, b: string): Promise<boolean> {
  ...
  const key = await crypto.subtle.importKey('raw', enc.encode('semo-orders-preview'), { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']);
...
Deno.serve(async (req) => {
  ...
  return json({ ...payload, message_ru, orders_page_url: ordersPageUrl }, 200, req);
});
